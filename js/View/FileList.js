...
class FileListView {
    constructor(boundingEl, dirService){
        this.dir = dirService;
        this.el = boundingEl;
        //Subcribe to DirService updates here too
    ...
    update( collection ) {
        this.el.innerHTML = `<li class="file-list__li file-list__head">
                            ...
                            <span class="file-list__li__time">Modified</span> </li>`
        collection.forEach((fInfo) => {
            ...
            <span class="file-list__li__size">${fInfo.stats.size}</span>
            <span class="file-list__li__time">${FileListView.formatTime(fInfo.stats.mtime)}</span> </li>`)
        });
        this.bindUi();
    }
        ...
        Array.from( this.el.querySelectorAll(".file-list__li"))
            .forEach((el) => {
                el.addEventListener("click", (e) => {
                    e.preventDefault();
                    nw.Shell.openItem(this.dir.getFilePath(el.dataset.file));
                }, false)
            });
    }
}
